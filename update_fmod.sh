...
mkdir -p "$TMPDIR/mac"
hdiutil attach "${PREFIX}mac-installer.dmg" -mountpoint "$TMPDIR/mac"
...
mkdir -p "$TMPDIR/ios"
hdiutil attach "${PREFIX}ios-installer.dmg" -mountpoint "$TMPDIR/ios"
...
tar xzf "${PREFIX}linux.tar.gz" -C "$TMPDIR/linux" --strip-components 1
...
mkdir -p "$TMPDIR/win"
unar "${PREFIX}win-installer.exe" -o "$TMPDIR/win"
cp "$TMPDIR/win"/*/*/api/core/lib/x64/fmod.dll "$REPO/fmod/res/x86_64-win32/fmod.dll"
cp "$TMPDIR/win"/*/*/api/studio/lib/x64/fmodstudio.dll "$REPO/fmod/res/x86_64-win32/fmodstudio.dll"
cp "$TMPDIR/win"/*/*/api/core/lib/x86/fmod.dll "$REPO/fmod/res/x86-win32/fmod.dll"
cp "$TMPDIR/win"/*/*/api/studio/lib/x86/fmodstudio.dll "$REPO/fmod/res/x86-win32/fmodstudio.dll"

echo -e "${YELLOW}Updating HTML5 FMOD...${NC}"
mkdir -p "$TMPDIR/html5"
unzip "${PREFIX}html5.zip" -d "$TMPDIR/html5"
